...
- A **View changelog & releases** link that opens the product changelog on the Openship website.
...
updates the CLI — which bundles the API server — to the latest release.
...
Openship checks GitHub, and if there's a newer version it updates itself in place. If you used the
**install script** (the default), it re-downloads the checksum-verified release bundle and swaps it in —
refreshing the Node runtime first if a release ever needs a newer one. If you installed the **npm package**
instead, it reinstalls with your package manager (**bun** if you have it, otherwise **npm**). If you're
already current, it just says so and stops.
...
Installed the npm package? Force a specific package manager if you need to (ignored for install-script
installs, which always update from the verified release bundle):
...
take it down). See [Installation](https://openship.io/docs/getting-started/installation) for the full service commands.
...
`openship update` either re-downloads the verified release bundle (install-script installs) or reinstalls
the global package with bun or npm. If that step errors, it prints the exact command to run by hand — copy
it and run it yourself. Then restart with `openship up`.
</Callout>

<Callout title="The update stopped, naming POSTGRES_PASSWORD or BETTER_AUTH_SECRET" type="error">
A Compose install keeps its generated secrets only in `~/.openship/compose/.env`. If an update can't read that
file while the database volume is still present, it stops **without changing anything** rather than generating
values that couldn't work — most often because it ran as a different user (`sudo` reads `/root/.openship`). Re-run
as the user that owns the install, or restore the file (the previous one is kept at `.env.bak`). Full steps:
[Troubleshooting → openship up refuses to regenerate secrets](https://openship.io/docs/troubleshooting#symptom-openship-up-refuses-to-regenerate-secrets).
</Callout>
...
<Cards>
  <Card title="Installation" href="https://openship.io/docs/getting-started/installation" description="Every way to install Openship — and the full openship up / stop service commands." />
  <Card title="How Openship runs your apps" href="https://openship.io/docs/architecture/runtime-model" description="What the persistent service actually does behind the scenes." />
  <Card title="Roll back or redeploy" href="https://openship.io/docs/guides/rollback-redeploy" description="Undo a bad deploy or ship the same code again." />
</Cards>
