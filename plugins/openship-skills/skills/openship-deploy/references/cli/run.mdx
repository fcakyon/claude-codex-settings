...
description: Start, stop, update, and health-check the Openship server itself from the terminal — up, stop, reset-admin-password, update, install, open, status, and doctor.
---
...
- **`openship`** (no arguments) is how most self-hosters run the **server** (API + dashboard) — a guided setup
  that creates your admin and installs it as a service. `openship up` is the same install *without* the prompts
  (headless / CI); `openship update` keeps either current.
...
|---|---|---|
| `--compose` | Install via Docker Compose (published images: postgres + redis + api + dashboard + edge on `:80/:443`). | default when Docker is present on Linux |
...
| `--host <addr>` | Bind the dashboard to this interface so an upstream reverse proxy or another LAN host can reach it (e.g. `0.0.0.0` or a LAN IP). The API stays on loopback. A concrete IP auto-trusts that browser origin for login; for `0.0.0.0` or a domain also pass `--public-url` (or set `OPENSHIP_EXTRA_TRUSTED_ORIGINS`). | `127.0.0.1` |
...
| `--admin-password <pw>` | Admin password (min 8). Prefer the `OPENSHIP_ADMIN_PASSWORD` env var (keeps it out of shell history). | — |
...
| `--edge <action>` | `migrate` · `takeover` · `cancel` — pre-answer the `:80/:443` takeover (custom domain / Compose). | prompt, else cancel |
| `--reset-secrets` | Compose only: generate fresh secrets even though an existing database volume is present. Realigns the database password, but **every stored environment variable becomes permanently unreadable**. Only for an install whose `.env` is genuinely gone. | refuse instead |
...
<Callout title="Re-running `up` over a Compose install keeps its secrets — or stops" type="warn">
A Compose install's generated secrets (`POSTGRES_PASSWORD`, `BETTER_AUTH_SECRET`, `INTERNAL_TOKEN`) live only in
`~/.openship/compose/.env`, and re-running `openship up` reuses whatever it reads there. If that file can't be
read (deleted, restored from a partial backup, or you're running as a **different user** — `sudo` reads
`/root/.openship`, not your own) while the database volume is still present, `up` **refuses and changes nothing**:
a new `POSTGRES_PASSWORD` can never match a volume Postgres already initialised, and a new `BETTER_AUTH_SECRET`
can't decrypt the environment variables in the database. Put the original `.env` back — each run keeps the file it
replaced at `~/.openship/compose/.env.bak` — or re-run as the user that owns it. `--reset-secrets` overrides the
refusal and accepts the loss. `openship up --dry-run` names every secret it would generate rather than preserve.
</Callout>

<Callout title="Login rejected after binding remotely?" type="warn">
Openship checks the browser's `Origin` against a trusted list. When you bind to `0.0.0.0` or a domain **without**
...
`http://IP:port` origin is trusted automatically), or setting `OPENSHIP_EXTRA_TRUSTED_ORIGINS` to the exact
...
openship reset-admin-password --password 'new-strong-pass' --email you@example.com
```
...
[the auth commands](https://openship.io/docs/cli/access).
...
see [the auth commands](https://openship.io/docs/cli/access) and `openship up` above.
</Callout>
