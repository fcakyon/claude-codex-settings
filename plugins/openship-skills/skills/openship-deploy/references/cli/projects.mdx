...
<Callout title="Before you start">
  Log in once with `openship login`, and (optionally) run `openship init` in a project directory so
  deploy commands know which project to act on. Add `--json` to any command for machine-readable
  output (stdout becomes a clean data stream). Where a command wants a project, most accept a
  `proj_…` id; `openship service` also accepts a slug or name.
</Callout>
...
# A local Compose source is scanned by the API and its services are created now.
openship project create --name "My Stack" \
  --local-path /opt/apps/my-stack --type services
```
...
| `--server <id>`           | Bind the new project to a registered server (`openship server list`). |

`project get <id>` prints the resolved `deployTarget` and `serverId`, so the
binding can be verified before the first deployment.

`--local-path` names a directory readable by the Openship API process (the
control-plane machine/container), not merely by the terminal running the CLI.
Local Compose imports use the same scanner and service persistence path as the
dashboard; a later deploy re-reads that directory and performs the normal
...
[environment variables guide](https://openship.io/docs/guides/environment-variables) for how these flow into builds and runtime.
...
[auto-deploy guide](https://openship.io/docs/guides/auto-deploy) for the full flow.
...
<Callout title="Two ways to add a domain">
  `project connect` and [`domain add`](#openship-domain) both attach a custom domain to a project
  and return the records to create. `connect` is the quickest one-liner; the `domain` command group
  adds verify, renew, and primary-selection on top. See the [custom domains
  guide](https://openship.io/docs/guides/custom-domains).
</Callout>
...
[sleep mode & resources guide](https://openship.io/docs/guides/sleep-mode-resources).
...
instance; see the [cloud connect & migration guide](https://openship.io/docs/guides/cloud-connect-migration).
...
[logs & monitoring guide](https://openship.io/docs/guides/logs-monitoring).
...
For the bigger picture, see the [multi-service compose guide](https://openship.io/docs/guides/compose-multi-service).
...
must be installed locally. Compose has already resolved environment expressions
in this path; Openship records that normalized provenance so a literal dollar
escaped with `$$` is not expanded a second time during deployment.
...
<Callout title="service exec is not available yet" type="info">
  `openship service exec` is stubbed — an interactive terminal needs a WebSocket client the CLI
  doesn't yet ship. Use the dashboard's service terminal for now.
</Callout>
...
[Domains API reference](https://openship.io/docs/api/domains), and the full walkthrough lives in the
[custom domains guide](https://openship.io/docs/guides/custom-domains).
...
<Callout title="Adding a domain doesn't make it live" type="warn">
  `add` (and `preview`) return the records to create at your DNS provider. The domain only goes live
  after the records resolve and `verify` succeeds — for a domain that isn't propagating, re-check
  the records against `preview` and retry. Certificates are then provisioned automatically.
</Callout>
